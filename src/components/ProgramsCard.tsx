...
export function ProgramCard({ session, onReserve, onView }: ProgramCardProps) {
    return (
        <div className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-1">{session.type}</h3>
            {session.theme && (
                <p className="text-indigo-600 font-medium mb-2">{session.theme}</p>
            )}
            {session.subType && (
                <p className="text-sm text-gray-600 mb-2">{session.subType}</p>
            )}
                ...
                <div className="flex flex-wrap gap-2">
                    {session.animateurs.map((animateur) => (
                        <div key={animateur.id} className="flex items-center space-x-2">
                            <img
                                src={`${animateur.photo}?w=40&h=40&fit=crop`}
                                alt={animateur.name}
                                className="w-8 h-8 rounded-full object-cover"
                            />
                            <div>
                                <p className="text-sm font-medium">{animateur.name}</p>
                                <p className="text-xs text-gray-500">{animateur.role}</p>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div className="flex items-center text-sm text-gray-600 mb-4">
                ...
                <span>{session.participants.length} / {session.availableSpots} participants</span>
            </div>
